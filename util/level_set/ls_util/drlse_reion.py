...
def drlse_region(phi_0, I, lmda, mu, alfa, epsilon, timestep, iters, potentialFunction):  # Updated Level Set Function
    """
...
def drlse_region_interaction(phi_0, I, P,lmda, mu, alfa, beta, 
        epsilon, timestep, iters, potentialFunction):  # Updated Level Set Function
    """
...
%              and p2 (double-well), respectively.
    ...
    for k in range(iters):
        ...
        edgeTerm = diracPhi * curvature
        phi = phi + timestep * (mu * distRegTerm + lmda * edgeTerm - alfa * areaTerm + beta * lu)
    return phi

def drlse_region_interaction2(phi_0, I, D,lmda, mu, alfa,
        epsilon, timestep, iters, potentialFunction):  # Updated Level Set Function
    """

    :param phi_0: level set function to be updated by level set evolution
    :param I: the input image to be segmented
    :param D: the distance transform of seed points
    :param lmda: weight of the weighted length term
    :param mu: weight of distance regularization term
    :param alfa: weight of the weighted area term
    :param beta: weight of user interaction term
...
def drlse_region_edge_interaction(phi_0, I, G, P, lmda, mu, alfa, beta,
        epsilon, timestep, iters, potentialFunction):  # Updated Level Set Function
    """

    :param phi_0: level set function to be updated by level set evolution
    :param I: the input image to be segmented
    :param G: the gradient of image
    :param P: the probability of being foreground, based on user interaction
    :param lmda: weight of the weighted length term (edge term)
    :param mu: weight of distance regularization term
    :param alfa: weight of the weighted area term
    :param beta: weight of user interaction term
    :param epsilon: width of Dirac Delta function
...
def Dirac(x, sigma):
    f = (1.0 / 2 / sigma) * (1 + np.cos(np.pi * x / sigma))
    b = (x <= sigma) & (x >= -sigma)
    return f * b
